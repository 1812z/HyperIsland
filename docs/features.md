...
在 **设置 > 过滤规则** 中，可以按前台应用**单独控制**通知在特殊状态下的表现。

在 **设置 > 其他 > 过滤规则** 中，可以按系统场景**全局控制**通知在特殊状态下的表现。
...
::: warning
关闭焦点通知时，超级岛由**系统界面**代发。默认配置中的「拦截悬浮通知」默认开启，开启后会自动拦截已成功上岛通知的原始悬浮通知，通知栏中的原始通知仍正常保留。
:::
...
- **黑名单模式**：包含关键词则不显示为超级岛
- **白名单模式**：仅显示包含白名单关键词且不包含黑名单关键词的通知；同时命中时以黑名单为准
...
- 大岛、小岛和展开态可分别选择默认、高斯模糊、高光玻璃、液态玻璃或柔光玻璃
- 小岛和展开态可跟随大岛，也可独立调节光影、折射、背景、混色与高光参数
- 柔光玻璃在 HyperOS 4 的大岛、小岛和展开态均复用系统原生 Bionics 材质接口；HyperOS 3 自动回退为高光玻璃
- 自定义背景与玻璃材质互斥；HDR 高光、液态玻璃采样和陀螺仪光效仍为全局设置
...
在 **设置 → Hook 拓展 → 屏幕录制** 中开启「录屏上岛」，可用超级岛和独立 Miuix 弹窗代替官方录屏悬浮窗，并支持暂停、继续和结束录制。录制前可临时开启「动态照片」，动态照片录制最长30秒并会自动结束，随后在原录屏目录生成动态照片；封装成功后删除原 MP4，失败时保留 MP4。总开关变更后需要重启屏幕录制应用；「立刻录制」支持热加载，开启后点击通知栏磁贴会跳过询问弹窗直接录制。

该功能可能仅支持部分版本，当前适配版本为 [4.16.2.6.1](https://1848933255.share.123pan.cn/123pan/9T69vd-ej0wd?pwd=kAf1#)。
