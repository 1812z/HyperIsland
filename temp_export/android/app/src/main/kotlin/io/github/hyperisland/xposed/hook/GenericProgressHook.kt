...
class GenericProgressHook : IXposedHookLoadPackage {
    ...
    companion object {
        ...
        /** 通用字符串设置懒加载，带缓存。 */
        private fun querySettingString(
            context: Context,
            prefKey: String,
            default: String = ""
        ): String {
            val uri = Uri.parse("content://$SETTINGS_AUTHORITY/$prefKey")
            return try {
                context.contentResolver.query(uri, null, null, null, null)?.use { cursor ->
                    if (!cursor.moveToFirst()) return default
                    val valueIndex = cursor.getColumnIndex("value").takeIf { it >= 0 } ?: 0
                    when (cursor.getType(valueIndex)) {
                        Cursor.FIELD_TYPE_STRING -> cursor.getString(valueIndex) ?: default
                        Cursor.FIELD_TYPE_INTEGER -> cursor.getLong(valueIndex).toString()
                        Cursor.FIELD_TYPE_FLOAT -> cursor.getDouble(valueIndex).toString()
                        else -> cursor.getString(valueIndex) ?: default
                    }
                } ?: default
            } catch (e: Exception) {
                XposedBridge.log("HyperIsland[Generic]: provider query failed for $prefKey: ${e.message}")
                default
            }
        }

        private fun querySettingBoolean(
            context: Context,
            prefKey: String,
            default: Boolean
        ): Boolean {
            return when (val raw = querySettingString(context, prefKey, if (default) "1" else "0")) {
        ...
        private fun loadChannelStringSetting(
            ...
            val value = querySettingString(context, prefKey, default)
        ...
        private fun loadBooleanSetting(
            ...
            val value = querySettingBoolean(context, prefKey, default)
        ...
        fun loadChannelTemplate(
            ...
            val template = querySettingString(context, key)
                .takeIf { it.isNotBlank() } ?: NotificationIslandNotification.TEMPLATE_ID
            cachedTemplates[cacheKey] = template
            return template
        }

        private fun loadWhitelist(context: Context): Map<String, Set<String>> {
            cachedWhitelist?.let { return it }
            val csv = querySettingString(context, "pref_generic_whitelist")
            val map = csv.split(",")
                .map { it.trim() }
                .filter { it.isNotBlank() }
                .associate { pkg ->
                    val channelCsv = querySettingString(context, "pref_channels_$pkg")
    ...
    private fun handleSbn(sbn: StatusBarNotification, lpparam: XC_LoadPackage.LoadPackageParam) {
        try {
            ...
            val defaultFirstFloat = loadBooleanSetting(
                context,
                "global:default_first_float",
            ...
            val defaultMarquee = loadBooleanSetting(
                context,
                "global:default_marquee",
                "pref_default_marquee",
                false
            )
            val defaultFocusNotif = loadBooleanSetting(
                context,
                "global:default_focus_notif",
