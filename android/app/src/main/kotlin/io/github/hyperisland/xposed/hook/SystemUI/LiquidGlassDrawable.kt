...
internal class LiquidGlassDrawable(
    ...
    private val hdrHighlight = HdrHighlightSurface(context, host)
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (config.enabled) {
            ...
            hdrHighlight.update(
                ...
                lightX,
                lightY,
                (height * config.edgeWidth).coerceAtLeast(1f),
                config.highlight * drawableAlpha,
            )
        } else {
            hdrHighlight.hide()
        }
    ...
    override fun setAlpha(alpha: Int) {
        ...
        if (drawableAlpha <= 0f) hdrHighlight.hide()
        updateCaptureState()
        invalidateSelf()
    }
    ...
    private class HdrHighlightSurface(
        context: Context,
        host: View,
        ...
        fun update(
            ...
            val nextWidth = kotlin.math.ceil(bounds.width()).toInt().coerceAtLeast(1)
            val nextHeight = kotlin.math.ceil(bounds.height()).toInt().coerceAtLeast(1)
            val nextX = hostLocation[0] - rootLocation[0] + bounds.left
            val nextY = hostLocation[1] - rootLocation[1] + bounds.top
            ...
            edgeWidth = rimWidth
            intensity = highlight
            overlay.visibility = if (highlight > 0f) View.VISIBLE else View.INVISIBLE
            drawFrame()
        }
        ...
        private fun drawFrame() {
            ...
            val canvas = runCatching {
                ...
                surfaceClass.getDeclaredMethod("lockHardwareWideColorGamutCanvas").apply {
                    isAccessible = true
    ...
    private companion object {
        ...
        val HDR_HIGHLIGHT_SHADER = """
            ...
            half4 main(float2 fragCoord) {
                ...
                if (distance > 0.0 || uIntensity <= 0.0) return half4(0.0);
